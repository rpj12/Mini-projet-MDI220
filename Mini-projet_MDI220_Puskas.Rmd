...
print(floor(281/3.1)+1)

```


Application numérique : il faut au moins 91 donnees pour rejeter HO. 
...
```{r}
######PLOT DE LA FONCTION PUISSANCE A CHANGER########

n=length(discoveries)
lambda=seq(3,4,0.1)
beta=numeric(length(lambda))

for (i in (1:length(lambda)))  {
  beta[i] = 1 - ppois(c,lambda[i]*n)
}
plot(lambda,beta,type="n", ylim = 0:12)
lines(smooth.spline(lambda,beta),col="red")


beta = 1 - ppois(c,3.5*n)
nb=n
